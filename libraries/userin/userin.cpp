...
namespace {
   ...
   constexpr size_t max_ssid = 20;
   constexpr size_t max_password = 20;
...
namespace {
   ...
   constexpr char eeprom_magic[] = "XUHBoFI";
   constexpr size_t eeprom_magic_size = sizeof(eeprom_magic)/sizeof(eeprom_magic[0]);
       ...
       for (size_t i = 0; i < max_password; ++i){
        ...
        if ( ch == '\0'){
            if ( i > 0){
              ...
              Serial.println("invalid info in eeprom : STOP"); for(;;){;}
            }
        }else{
          password[i] = ch;
        }
...
bool check_do_setup()
{
  ...
  //delay here so that  commands can be entered
  //eg to change network and passwd
